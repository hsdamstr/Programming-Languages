...
DeclList : IdentifierList T_COLON Type T_SEMICOLON
		{   
			...
			dlinkApply2($1,(DLinkApply2Func)addIdToSymtab, (Generic)$3, (Generic) symStack); //apply to specific symtable on stack
...
IdentifierList : T_IDENTIFIER
		{
			int symTabIndex = SymIndex(currentSymtab(symStack),$1);		
			$$ = dlinkListAlloc(NULL);
	       ...
	       | IdentifierList T_COMMA T_IDENTIFIER
		{
			...
			symTabIndex = SymIndex(currentSymtab(symStack),$3);
...
Factor          : Variable
		...
		| T_IDENTIFIER T_LPAREN T_RPAREN{
			$$ = emitCallFunction(instList,$1); //call function allocates a register 
			emitAfterFunctionAssignment(instList, $$);
...
Variable        : T_IDENTIFIER
		{			int symIndex = SymQueryIndex(currentSymtab(symStack),$1);
					if(symIndex == -1){ //if sym query returns -1 then check global symtab
						symIndex = SymQueryIndex(lastSymtab(symStack),$1);
						$$ = emitComputeVariableAddress(instList, symIndex, symStack, lastSymtab(symStack)); //should be global symtab
					}
					else{
						$$ = emitComputeVariableAddress(instList, symIndex, symStack, currentSymtab(symStack));
					}
		}
                | T_IDENTIFIER T_LBRACKET Expr T_RBRACKET    
               	{
					...
					if(symIndex == -1){  //if sym query returns -1 then check global symtab
						...
						$$ = emitComputeArrayAddress(instList, symIndex,$3, symStack, lastSymtab(symStack));
               			...
               			$$ = emitComputeArrayAddress(instList, symIndex,$3, symStack, currentSymtab(symStack));
					}
				}
